...
#baseline
#MODEL_PATH = '/coc/flash3/nwarner30/image_editing/affordance-insertion/logs/2024-04-21T22-51-01_affordance/checkpoints/epoch=000390.ckpt'
#avg-distance-> set env variable

#MODEL_PATH = '/coc/flash3/nwarner30/image_editing/affordance-insertion/logs/2024-04-24T16-10-50_affordance/checkpoints/epoch=000365.ckpt'
#os.environ['DISTANCE_CHECK_MODE'] = 'check_average'

MODEL_PATH = '/coc/flash3/nwarner30/image_editing/affordance-insertion/logs/2024-04-24T16-34-24_affordance/checkpoints/epoch=000240.ckpt'
os.environ['DISTANCE_CHECK_MODE'] = 'check_hip'
    ...
    with torch.no_grad():
        with model.ema_scope():
            for batch_idx, batch in tqdm(enumerate(dataset)):
                ...
                # can vary when we edit out distance, but works always
                actual_batch_size = batch['image'].shape[0]  # Get the actual batch size from the batch data
                ...
                for cfg_scale in [CFG_SCALE ]:
                    ...
                    for idx in range(1):
                        ...
                        cond_dict_sdm['crossattn']['refer_person_text'] = torch.zeros_like(model.cond_text_model.encode(['']*actual_batch_size)) # null captions
                        ...
                        cond_dict_sdm['crossattn']['frame_pose_target'] = model.cond_pose_model.encode(uncond_frame_pose_target).repeat(actual_batch_size, 1, 1)
                        cond_dict_sdm['crossattn']['frame_pose_refer'] = model.cond_pose_model.encode(uncond_frame_pose_refer).repeat(actual_batch_size, 1, 1)
